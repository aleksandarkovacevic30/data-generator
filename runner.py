...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "company_params": {}, "beverage_params": {}, "scenarios": {}
}
...
GEN_MAP = {"company": CompanyGenerator, "beverage": BeverageGenerator}

def generate_batch(domain: str, batch_size: int, cfg: Dict[str, Any]) -> List[Dict[str, Any]]:
    GenCls = GEN_MAP.get(domain)
    if GenCls is None:
        raise RuntimeError(f"Domain '{domain}' not available")
    g = GenCls()
    scen = cfg.get("scenarios") or {}
    if scen:
        try: g.set_scenarios(scen)
        except Exception: pass
    params = cfg.get(f"{domain}_params") or {}
    for k, v in params.items():
        if hasattr(g, k): setattr(g, k, v)
    return g.generate_batch(int(batch_size))
...
def send_to_sink(records: List[Dict[str, Any]], sink_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    start = time.time()
    last_exc = None
    with httpx.Client(timeout=httpx.Timeout(connect=5.0, read=30.0, write=10.0, pool=5.0),
                      follow_redirects=True, trust_env=trust_env) as c:
            ...
            try:
                ...
                if resp is not None and resp.status_code < 400:
                    append_send_log({
                        ...
                        "status": resp.status_code, "duration_s": round(time.time()-start, 3),
                ...
                last_exc = e; time.sleep(0.25)
    curl = build_curl(url, bearer, payload_batch if mode=="batch" else [payload_batch[0]])
    append_send_log({
        "source": "runner", "domain": sink_cfg.get("domain") or "unknown",
        "count": len(payload_batch), "mode": mode, "ok": False,
        "status": 0, "duration_s": round(time.time()-start, 3),
        ...
        try:
            ...
            if cfg.get("running", False):
                ...
                try:
                    ...
                    heartbeat(); time.sleep(interval); continue
                sink_cfg = cfg.get("sink") or {}
                if sink_cfg.get("enabled", False):
                    result = send_to_sink(batch, sink_cfg)
                    if result.get("ok"):
                        save_state({"last_send_ok": True, "last_send_error": None,
                                    "last_send_status": result.get("status"),
                                    "last_send_curl": None, "last_row": batch[-1] if batch else None,
                                    "last_send_body": result.get("body")})
                    else:
                        save_state({"last_send_ok": False, "last_send_error": result.get("error"),
                                    "last_send_status": result.get("status"),
                    ...
                    save_state({"last_send_ok": None, "last_send_error": None,
